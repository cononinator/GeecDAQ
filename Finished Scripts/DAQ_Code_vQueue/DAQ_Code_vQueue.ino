...
void setup() {
  ...
  adcSetup();
  Serial1.println("ADC Setup");

  sdSetup();
  Serial1.println("SD setup");
...
void readADC( void * parameter )
{
    for(;;){
      ...
      t.time = millis();
      t.iBatt = 10*(ads.computeVolts(adc0)/1000.0 - 0.493);
      t.iMot = 10*(ads.computeVolts(adc1)/1000.0- 0.493);
      t.vMot = 7.51*(ads.computeVolts(adc2))/1000.0;
      t.vBat = 9.01*(ads.computeVolts(adc3))/1000.0; 
        
      String iB = String(t.iBatt,3);
      String iM = String(t.iMot,3);
      String vB = String(t.vBat,3);
      String vM = String(t.vMot,3);
      String message = String(t.time) + "," + iB + "," + iM + "," + vB + "," + vM + "," + String(t.speed, 1) + "," + t.GPS + "\n";
      Serial1.print(message);
      xQueueSend(buffer, (void *) &message, 0); //Send pointer to queue if queue full do nothing
...
void SaveToSDTask( void * parameter)
{
  for(;;){
    ...
    while(xQueueReceive(buffer, &receivedMessage, 0)){

            Serial1.println(receivedMessage);
            appendFile(file, receivedMessage);
    }
    file.close();
