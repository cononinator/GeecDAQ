...
void setup() {
  ...
  xTaskCreate(
                    printSpeed,          /* Task function. */
                    "printSpeed",        /* String with name of task. */
                    10000,            /* Stack size in bytes. */
                    NULL,             /* Parameter passed as input of the task */
...
String setNextFileName(String prefix) {
  ...
  while (true) {
    ...
    if (name.startsWith("testd") && name.endsWith(".txt")) {
...
void printSpeed( void * parameter ){
  for (;;){
    Serial.println(t.speed);
