...
void readADC( void * parameter )
{
    for(;;){
      ...
      vTaskDelay(pdMS_TO_TICKS((93)));  //Delay Can be removed or added to change speed
    }
...
String setNextFileName(String prefix) {
  String next = "";
  int highest = -1; // Keep track of highest number after prefix
  int prefixLength = prefix.length() - 1;

  // Open root directory on SD card
  File root = SD.open("/");
  while (true) {
    File entry = root.openNextFile();
    if (!entry) {
      break; // Exit loop once all files have been checked
    }

    // Check if filename begins with prefix
    ...
    if (name.startsWith(prefix.substring(1, prefixLength + 1)) && name.endsWith(".txt")) {
      ...
      if (number > highest) {
        highest = number; // Found new highest number
      }
    }
    entry.close();
  }

  highest++; // Increment to set as new file number
  ...
  filename = next; // Set the global file name
  return next;
}
